...
function draw() {
  ...
  for(let j = 0; j < numPoints; j++){
    ...
    let transform = random(Sierpinski.funcs);
  ...
  //   let postTrans = P0.apply(x,y)
  //   x = postTrans[0];
  //   y = postTrans[1];         
  // //  APPLY FINAL TRANSFORM HERE on output of post transform

  //   let finalTrans = F0.apply(x,y)
  //   let finalX = finalTrans[0];
  //   let finalY = finalTrans[1];
    
    let scaledX = Math.floor(x * w * 0.5 + (w/2)); // scale and translate to viewport size (scaling should be done after ALL transforms)
    let scaledY = Math.floor(y * h * 0.5 + (h/2));
  ...
  for (i = 0; i < h; i++){
    for(j = 0; j < w; j++){
      ...
      let c = color(_pixels[channels*(i*w + j)], _pixels[channels*(i*w + j) + 1], _pixels[channels*(i*w + j) + 2], Math.floor(255*alpha));
      stroke(c);
